...
// ============================================
// Same as equity NODE3 but includes yield curve chart embed.

var briefText = $input.first().json.briefText;
var subjectLine = $input.first().json.subjectLine;
var chartUrl = $input.first().json.chartUrl;
...
for (var b = 0; b < lines2.length; b++) {
  ...
  if (line2.indexOf('<tr>') === 0) {
    if (!inTable) { result2.push('<table style="border-collapse:collapse;margin:10px 0;width:100%;max-width:520px;">'); inTable = true; }
    result2.push(line2);
  } else {
...
// Insert yield curve chart after Funding section
if (chartUrl) {
  var chartImg = '<div style="margin:12px 0;text-align:center;"><img src="' + chartUrl + '" alt="Short-End Yield Curve" style="max-width:100%;width:600px;height:auto;border:1px solid #e5e7eb;border-radius:4px;" /></div>';
  var fundingIdx = html.indexOf('Funding');
  if (fundingIdx === -1) fundingIdx = html.indexOf('Liquidity');
  if (fundingIdx !== -1) {
    var insertIdx = html.indexOf('</h', fundingIdx);
    if (insertIdx !== -1) {
      var closeTag = html.indexOf('>', insertIdx) + 1;
      html = html.slice(0, closeTag) + chartImg + html.slice(closeTag);
    }
  } else {
    html = chartImg + html;
  }
